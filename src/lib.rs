...
pub struct TokenValidator {
    ...
    firebase_project_issuer: String,
    firebase_public_keys_jwk_url: String,
}

impl TokenValidator {
    pub async fn validate(&self, token: &str) -> Result<FirebaseClaims, ValidationError> {
        let validation = Validation {
            aud: Some(HashSet::from([self.firebase_project_id.to_owned()])),
            iss: Some(self.firebase_project_issuer.to_owned()),
        ...
        let mut response = client.get(self.firebase_public_keys_jwk_url.to_owned()).send().await?;
